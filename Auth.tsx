...
export const Auth: React.FC<AuthProps> = ({ lang, onLogin, onGuest, onNavigate }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // OAuth will redirect the page, so we don't necessarily reset isGoogleProcessing here
    } catch (err: any) {
                    ...
                    <div className="space-y-4 pt-1">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button 
                          ...
                          disabled={isProcessing || isGoogleProcessing}
                          className="py-3.5 bg-[#0f121e] border border-white/5 rounded-2xl flex items-center justify-center gap-2.5 text-slate-500 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest disabled:opacity-50"
                        >
                          ...
                          {isGoogleProcessing ? 'HANDSHAKE...' : t.google}
                        </button>
                        <button 
                          type="button" 
                          onClick={onGuest} 
