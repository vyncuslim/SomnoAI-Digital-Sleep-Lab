...
export const Auth: React.FC<AuthProps> = ({ lang, onLogin, onGuest, onNavigate, isAdminFlow = false }) => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(err.message || "Admin Access Denied");
    } finally {
      setIsLoggingIn(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-[#020617]">
      ...
      <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-lg space-y-10 text-center relative z-10">
        <div className="flex flex-col items-center gap-6">
          <div className={`w-28 h-28 rounded-full border flex items-center justify-center transition-all duration-700 ${isAdminFlow ? 'bg-rose-600/10 border-rose-500/30' : 'bg-indigo-600/10 border-indigo-500/10'}`}>
        ...
        <GlassCard className={`p-10 rounded-[4rem] border-white/10 ${isAdminFlow ? 'shadow-2xl shadow-rose-950/20' : ''}`}>
          <AnimatePresence mode="wait">
            {!isAdminFlow ? (
              <m.div key="user" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
              ...
              <m.div key="admin" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                 <form onSubmit={handleEmailLogin} className="space-y-4">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-950/60 border border-white/5 rounded-full px-16 py-5 text-xs text-white uppercase font-black tracking-widest outline-none focus:border-rose-500/50"
                        required
                      />
                    </div>
                    <div className="relative">
          ...
          {localError && (
            <m.div className="mt-6 p-4 bg-rose-500/10 rounded-2xl border border-rose-500/20 text-rose-400 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3">
