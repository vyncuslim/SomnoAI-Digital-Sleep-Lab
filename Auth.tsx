...
const BiometricSwitch = ({ active = false }: { active?: boolean }) => (
  <div className="flex items-center">
    <div className={`w-10 h-6 rounded-full border border-white/10 flex items-center px-0.5 bg-black/40 relative overflow-hidden transition-all duration-500 ${active ? 'border-emerald-500/40 shadow-[0_0_12px_rgba(16,185,129,0.3)]' : ''}`}>
      ...
      <div className="absolute right-1 top-1.5 opacity-10 pointer-events-none">
...
export const Auth: React.FC<AuthProps> = ({ lang, onLogin, onGuest, onNavigate }) => {
    ...
    try {
      if (formType === 'reset') {
        ...
        if (formType === 'register') {
          const { error: signUpErr } = await authApi.signUp(email.trim(), password);
          if (signUpErr) throw signUpErr;
          setMessage(lang === 'zh' ? "注册成功！请查收邮件完成验证。" : "Registration successful! Verify via email.");
        } else {
  ...
  const executeOtpVerify = async (fullOtp?: string) => {
    ...
    setIsProcessing(true);
    try {
      const { data, error: verifyErr } = await authApi.verifyOTP(email.trim(), token);
      if (verifyErr) throw verifyErr;
      if (data.session) onLogin();
    } catch (err: any) {
      setError(err.message || "Handshake Rejected.");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-[#020617] relative overflow-hidden">
      ...
      {/* 顶部标识 */}
      <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8 relative z-10 space-y-4">
        ...
        <div>
          ...
          <p className="text-slate-600 font-bold uppercase text-[9px] tracking-[0.4em] mt-1 opacity-80">{t.tagline}</p>
        </div>
      </m.div>

      {/* 登录卡片 */}
      <div className="w-full max-w-[420px] relative z-20">
        <div className="bg-[#050a1f]/90 backdrop-blur-3xl border border-white/[0.08] rounded-[3rem] p-1 shadow-2xl">
          <div className="p-8 md:p-10 space-y-8">
            
            {/* 模态切换 */}
            <div className="flex bg-black/50 p-1 rounded-full border border-white/5 relative">
              <button 
                onClick={() => { setAuthMode('otp'); setFormType('login'); setStep('input'); }}
                className={`flex-1 py-3.5 rounded-full text-[9px] font-black uppercase tracking-widest relative z-10 transition-colors duration-500 ${authMode === 'otp' ? 'text-white' : 'text-slate-500'}`}
              >
                OTP MODE
              </button>
              <button 
                onClick={() => { setAuthMode('password'); setFormType('login'); setStep('input'); }}
                className={`flex-1 py-3.5 rounded-full text-[9px] font-black uppercase tracking-widest relative z-10 transition-colors duration-500 ${authMode === 'password' ? 'text-white' : 'text-slate-500'}`}
              >
                PASSWORD MODE
              </button>
              <m.div 
              ...
              {step === 'input' ? (
                <m.div key="input" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-8">
                  ...
                  <div className="flex justify-center gap-10">
                    <button 
                      ...
                      className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${formType === 'login' ? 'text-indigo-400 border-indigo-400' : 'text-slate-800 border-transparent hover:text-slate-600'}`}
                    >
                      <LoginIcon size={12} /> LOGIN
                    </button>
                    <button 
                      onClick={() => setFormType('register')} 
                      className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${formType === 'register' ? 'text-indigo-400 border-indigo-400' : 'text-slate-800 border-transparent hover:text-slate-600'}`}
                    >
                  ...
                  <form onSubmit={handleMainAction} className="space-y-6">
                    <div className="space-y-4">
                      {/* 宣言 */}
                      <p className="text-[10.5px] text-slate-500 text-center leading-relaxed font-medium italic px-4">
                      ...
                      <div className="relative group">
                        ...
                        <input 
                          ...
                          className="w-full bg-[#0a0e1a] border border-white/5 rounded-full px-14 py-4 text-xs text-white focus:border-indigo-500/50 outline-none transition-all placeholder:text-slate-900 font-semibold"
                          required
                        />
                        <div className="absolute right-5 top-1/2 -translate-y-1/2 flex items-center gap-2.5">
                        ...
                        <div className="space-y-2">
                          ...
                          {formType === 'login' && (
                            <div className="text-right">
                              <button type="button" onClick={() => setFormType('reset')} className="text-[9px] font-black text-slate-800 hover:text-indigo-400 uppercase tracking-widest transition-colors">{t.forgotPassword}</button>
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <div className="space-y-4 pt-2">
                      <button 
                        ...
                        className="w-full py-5 bg-[#4f46e5] text-white rounded-full font-black text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3 active:scale-[0.98] transition-all shadow-xl hover:bg-[#5a50f0] disabled:opacity-50"
                      >
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <button 
                          ...
                          className="py-4 bg-[#0f121e] border border-white/5 rounded-2xl flex items-center justify-center gap-3 text-slate-500 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest"
                        >
                ...
                <m.div key="verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  <div className="text-center space-y-3">
                    ...
                    <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter leading-none">{t.handshake}</h2>
                  ...
                  <div className="flex justify-between gap-2 px-2">
                    {otp.map((digit, idx) => (
                      <input 
                        ...
                        className="w-10 h-14 bg-slate-950/60 border border-white/10 rounded-2xl text-2xl text-center text-white font-mono font-black focus:border-[#4f46e5] outline-none transition-all"
                      />
                    ))}
                  </div>
                  ...
                  <div className="space-y-4">
                    <button 
                      ...
                      className="w-full py-5 bg-[#4f46e5] text-white rounded-full font-black text-[10px] uppercase tracking-[0.4em] flex items-center justify-center gap-3 hover:bg-[#5a50f0] active:scale-[0.97] transition-all disabled:opacity-50"
                    >
                    ...
                    >
                      {cooldown > 0 ? `RETRY IN ${cooldown}S` : t.resend}
                    </button>
                  </div>
                </m.div>
              )}
            ...
            {/* 状态提示 */}
            {(error || message) && (
              <m.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`p-5 border rounded-3xl flex items-start gap-3 text-[10px] font-bold italic leading-relaxed ${error ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
          ...
          {/* 页脚帮助 */}
          <div className="pb-8 text-center border-t border-white/5 pt-6">
             <button className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-800 hover:text-indigo-400 transition-colors flex items-center gap-2 mx-auto justify-center group">
                {t.help}
             </button>
          </div>
        </div>
      ...
      {/* 版权信息 */}
      <footer className="mt-12 text-center space-y-4 opacity-40 hover:opacity-100 transition-all duration-700 pb-8">
        <p className="text-[8px] font-mono uppercase tracking-[0.5em] text-slate-800 italic">© 2025 SOMNO LAB • NEURAL CORE</p>
      </footer>
    </div>
  );
};