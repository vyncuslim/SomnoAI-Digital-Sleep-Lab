...
export const AdminView: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  /**
   * 核心权限判定逻辑 (Hierarchy Check)
   */
  const getBlockPermissionStatus = (target: any) => {
    if (!currentAdmin) return { canAction: false, reason: 'WAITING_SYNC' };
    if (target.is_super_owner) return { canAction: false, reason: 'SUPER_OWNER_IMMUNE' };
    ...
    // Super Owner 可以封禁除了自己以外的任何人
    if (myRole === 'super') return { canAction: true };

    // Owner 可以封禁 admin 和 user，但不能封禁 owner
    if (myRole === 'owner') {
      if (targetRole === 'owner') return { canAction: false, reason: 'RANK_PARITY' };
      return { canAction: true };
    }

    // Admin 只能封禁 user，不能封禁 admin 或 owner
    if (myRole === 'admin') {
      if (targetRole === 'admin' || targetRole === 'owner') return { canAction: false, reason: 'RANK_INSUFFICIENT' };
      return { canAction: true };
    }

    return { canAction: false, reason: 'NO_CLEARANCE' };
  };
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1 md:space-y-2 text-left">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] italic">Authority:</span>
              <div className={`px-3 py-1 rounded-full border text-[8px] font-black uppercase tracking-widest ${currentAdmin?.is_super_owner ? 'bg-amber-500/10 border-amber-500/30 text-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : currentAdmin?.role === 'owner' ? 'bg-amber-400/10 border-amber-400/30 text-amber-400' : 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400'}`}>
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          <div key="loading" className="flex flex-col items-center justify-center py-48 gap-8">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.6em] text-slate-500 italic">Synchronizing Authority Matrix...</p>
          </div>
        ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {DATABASE_SCHEMA.map((stat, i) => (
                    <GlassCard key={i} className="p-8 rounded-[3rem] border-white/5">
                      ...
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-3">{stat.group || 'System Node'}</p>
                    </GlassCard>
                  ))}
                </div>
                  ...
                  <div className="lg:col-span-4">
                    <GlassCard className="p-10 rounded-[4rem] border-white/5 h-full flex flex-col justify-between gap-10">
                       <div className="space-y-6">
                          ...
                          <p className="text-[11px] text-slate-500 leading-relaxed italic">Direct telemetry link between GA4 data nodes and laboratory registry.</p>
                       </div>
                       <button onClick={handleManualSync} disabled={syncState === 'RUNNING'} className="w-full py-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full font-black text-[12px] uppercase tracking-[0.4em] transition-all flex items-center justify-center gap-4 shadow-xl italic active:scale-95 disabled:opacity-30">
                         {syncState === 'RUNNING' ? <Loader2 size={16} className="animate-spin" /> : <RefreshCw size={16} />} PULSE OVERRIDE
                       </button>
                    </GlassCard>
                  </div>
                </div>
            ...
            {activeTab === 'registry' && (
              <div className="space-y-8">
                <div className="relative max-w-xl mx-2">
                   ...
                   <input type="text" value={registrySearch} onChange={(e) => setRegistrySearch(e.target.value)} placeholder="Search Identification String..." className="w-full bg-slate-900/60 border border-white/10 rounded-full pl-16 pr-8 py-5 text-sm text-white focus:border-indigo-500/50 outline-none italic font-bold" />
                </div>
                    ...
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b border-white/5 bg-white/[0.02]">
                          <th className="px-8 py-6 text-[9px] font-black text-slate-500 uppercase tracking-widest italic whitespace-nowrap">Subject Identifier</th>
                          ...
                          <th className="px-8 py-6 text-[9px] font-black text-slate-500 uppercase tracking-widest italic whitespace-nowrap">Channel Security</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5">
                        {users.filter(u => u.email?.toLowerCase().includes(registrySearch.toLowerCase()) || u.full_name?.toLowerCase().includes(registrySearch.toLowerCase())).map((u) => {
                          const { canAction, reason } = getBlockPermissionStatus(u);
                          ...
                          return (
                            <tr key={u.id} className={`hover:bg-white/[0.01] transition-colors group ${u.is_blocked ? 'bg-rose-500/[0.01]' : ''}`}>
                                 ...
                                 <div className="relative w-32">
                                    {canEditRole ? (
                                      <select 
                                        ...
                                        className={`w-full bg-slate-900 border rounded-full px-4 py-2 text-[10px] font-black uppercase tracking-widest outline-none focus:border-indigo-500/50 appearance-none cursor-pointer hover:text-white transition-all italic ${roleConfig.color} ${roleConfig.border}`}
                                      >
                              ...
                              <td className="px-8 py-7">
                                 <div className="flex items-center gap-6">
                                    ...
                                    <button 
                                      ...
                                      title={!canAction ? `Action Forbidden: ${reason}` : (u.is_blocked ? 'Restore Link' : 'Sever Link')}
                                      className={`p-2.5 rounded-xl border transition-all active:scale-90 ${
                                        !canAction ? 'bg-slate-900 border-white/5 text-slate-800 cursor-not-allowed' :
                                        u.is_blocked 
                                          ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20' 
                                          : 'bg-rose-500/10 border-rose-500/20 text-rose-400 hover:bg-rose-500/20 shadow-lg shadow-rose-950/20'
                                      }`}
                                    >
                                      {modifyingUserId === u.id ? <Loader2 size={16} className="animate-spin" /> : u.is_blocked ? <Unlock size={16} /> : <Ban size={16} />}
                                    </button>
                                 </div>
                              </td>
                            </tr>
