...
import { 
  ...
  Zap, Fingerprint, Lock, Eye, EyeOff, User, ChevronLeft, Terminal
} from 'lucide-react';
...
export const Auth: React.FC<AuthProps> = ({ lang, onLogin, onGuest }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-[#020617] font-sans relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-transparent via-indigo-500/[0.02] to-transparent animate-pulse" />
      ...
      <div className="w-full max-w-[420px] space-y-8 relative z-10">
        {step === 'request' ? (
          <>
            {/* Nav Switcher */}
            <div className="bg-slate-900/60 p-1 rounded-full border border-white/5 flex relative shadow-2xl">
              ...
              <m.div 
                className="absolute top-1 left-1 bottom-1 w-[calc(33.33%-2px)] bg-indigo-600 rounded-full"
            ...
            <form onSubmit={handleAuthAction} className="space-y-6">
              ...
              {error && (
                <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex gap-3 text-rose-400">
              ...
              <button 
                ...
                className="w-full py-5 rounded-full bg-indigo-600 text-white font-black text-xs uppercase tracking-[0.4em] shadow-2xl active:scale-[0.98] transition-all hover:bg-indigo-500 shadow-indigo-600/20"
              >
                {isProcessing ? <Loader2 className="animate-spin" size={18} /> : <span>AUTHORIZE</span>}
              </button>
            </form>
          ...
          <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-10" >
            <div className="text-center space-y-3">
              ...
              <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest truncate px-4">
                SECURITY TOKEN DISPATCHED TO IDENTIFIER
              </p>
            </div>
