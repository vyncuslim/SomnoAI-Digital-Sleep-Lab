  ...
  <div className="flex items-center">
    <div className={`w-12 h-7 rounded-full border border-white/10 flex items-center px-1 bg-black/40 relative overflow-hidden transition-all duration-500 ${active ? 'border-emerald-500/40 shadow-[0_0_12px_rgba(16,185,129,0.3)]' : ''}`}>
      ...
      <m.div 
        ...
        className={`w-5 h-5 rounded-full relative z-10 flex items-center justify-center ${active ? 'bg-white shadow-[0_0_8px_rgba(255,255,255,1)]' : 'bg-slate-700'}`}
      >
        {active && <div className="w-2.5 h-2.5 bg-[#00f2fe] rounded-full blur-[1px]" />}
      </m.div>
    </div>
  </div>
);
...
const HexKeyIcon = ({ active }: { active: boolean }) => (
  <div className={`p-2 rounded-xl border border-white/5 bg-[#0f121e] flex items-center justify-center transition-all duration-700 ${active ? 'text-indigo-400 border-indigo-500/30 shadow-[0_0_10px_rgba(79,70,229,0.15)]' : 'text-slate-800'}`}>
      ...
      <div className="w-full max-w-[460px]">
        <div className="bg-[#050a1f]/90 backdrop-blur-3xl border border-white/[0.08] rounded-[3.5rem] p-1 shadow-2xl overflow-hidden">
          <div className="p-10 md:p-12 space-y-10 text-left">
              ...
              {step === 'input' ? (
                <m.div key="input" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-10">
                  ...
                  <div className="flex justify-center gap-12">
                    <button 
                      ...
                      className={`flex items-center gap-2 text-[12px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${formType === 'login' ? 'text-indigo-400 border-indigo-400' : 'text-slate-800 border-transparent hover:text-slate-600'}`}
                    >
                      <LoginIcon size={16} /> LOGIN
                    </button>
                    <button 
                      onClick={() => setFormType('register')} 
                      className={`flex items-center gap-2 text-[12px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${formType === 'register' ? 'text-indigo-400 border-indigo-400' : 'text-slate-800 border-transparent hover:text-slate-600'}`}
                    >
                      <RegisterIcon size={16} /> REGISTER
                    </button>
                  </div>

                  <form onSubmit={handleMainAction} className="space-y-8">
                    <p className="text-[12px] text-slate-500 text-center leading-relaxed italic px-8 font-medium">
                    ...
                    <div className="space-y-4 pt-2">
                      <button 
                        ...
                        className="w-full py-6 bg-[#4f46e5] text-white rounded-full font-black text-[12px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 active:scale-[0.98] transition-all shadow-2xl hover:bg-[#5a50f0] disabled:opacity-50"
                      >
                ...
                <m.div key="verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12">
                  <div className="text-center space-y-4">
                    <button onClick={() => setStep('input')} className="text-[11px] font-black text-indigo-400 uppercase flex items-center gap-2 mx-auto hover:text-indigo-300">
                      <ChevronLeft size={16} /> {t.back}
                    </button>
                    <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter leading-none">{t.handshake}</h2>
                    <p className="text-[12px] text-slate-600 font-medium italic truncate px-8">{t.dispatched} {email}</p>
                  </div>
                  <div className="flex justify-between gap-3 px-8">
                    {otp.map((digit, idx) => (
                      <input 
                        ...
                        className="w-12 h-16 bg-slate-950/60 border border-white/10 rounded-2xl text-3xl text-center text-white font-mono font-black focus:border-[#4f46e5] outline-none transition-all shadow-inner"
                      />
                    ))}
                  </div>
                  <div className="space-y-4">
                    <button 
                      ...
                      className="w-full py-6 bg-[#4f46e5] text-white rounded-full font-black text-[12px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 hover:bg-[#5a50f0] active:scale-[0.97] transition-all disabled:opacity-50 shadow-2xl"
                    >
            ...
            {(error || message) && (
              <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`p-6 border rounded-[2rem] flex items-start gap-4 text-[12px] font-bold italic leading-relaxed shadow-lg ${error ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
                <div className="mt-1">
                  {error ? <ShieldAlert size={20} /> : <ShieldCheck size={20} />}
                </div>
                <div className="space-y-1">
                  <p>{error || message}</p>
                  {message && !error && <p className="text-[10px] uppercase tracking-widest opacity-60">System identity validated.</p>}
                </div>
              </m.div>
            )}
          </div>
