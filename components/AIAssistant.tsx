...
import { 
  Send, User, Loader2, Trash2, Music, ExternalLink, Moon, Key, ShieldAlert, Camera, X, Scan, Zap, Eye
} from 'lucide-react';
...
export const AIAssistant: React.FC<AIAssistantProps> = ({ lang, data }) => {
  ...
  const [messages, setMessages] = useState<(ChatMessage & { sources?: any[], image?: string })[]>([]);
  const [input, setInput] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const [apiKeyMissing, setApiKeyMissing] = useState(false);
  
  // Facial Scanning State
    ...
    <div className="flex flex-col h-[calc(100vh-160px)] max-w-2xl mx-auto font-sans relative">
          ...
          {showScanner && (
            ...
            >
                ...
                <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
                  <div className="w-3/4 h-3/4 border-2 border-indigo-500/30 rounded-3xl relative overflow-hidden">
                    <m.div 
                      ...
                      className="absolute left-0 right-0 h-1 bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,1)]"
                    />
                    <div className="absolute top-4 left-4 text-indigo-400 flex items-center gap-2">
                       <Scan size={14} className="animate-pulse" />
                       <span className="text-[8px] font-black uppercase tracking-widest">Biometric Mapping...</span>
                    </div>
                  </div>
                </div>
                ...
                {isScanning && (
                   <div className="absolute inset-0 bg-indigo-600/20 backdrop-blur-sm flex items-center justify-center">
                      <div className="text-center space-y-4">
                         ...
                         <p className="text-xs font-black text-white uppercase tracking-widest italic">Analyzing Fatigue Markers...</p>
                      </div>
                   </div>
                )}
              </div>
              ...
              <div className="p-8 bg-slate-900 flex justify-between items-center">
                <button onClick={stopScanner} className="p-4 bg-white/5 rounded-full text-slate-400"><X size={20} /></button>
                <button 
                  onClick={captureFrame} 
                  disabled={isScanning}
                  className="px-10 py-4 bg-indigo-600 text-white rounded-full font-black text-[10px] uppercase tracking-widest shadow-xl flex items-center gap-3"
                >
                  <Zap size={16} fill="white" /> Execute Scan
                </button>
                <div className="w-12" />
              </div>
            </m.div>
              ...
              <div className={`p-5 rounded-[2rem] text-sm leading-relaxed shadow-xl text-left relative overflow-hidden ${msg.role === 'assistant' ? 'bg-slate-900/60 border border-white/5 text-slate-300' : 'bg-indigo-600 text-white'}`}>
                {msg.image && (
                   <div className="mb-4 rounded-2xl overflow-hidden border border-white/10">
                      <img src={msg.image} alt="Biometric Scan" className="w-full h-auto max-h-48 object-cover" />
                      <div className="p-2 bg-black/40 text-[8px] font-black uppercase text-indigo-400 flex items-center gap-2">
                         <Eye size={10} /> Facial Biometric Frame Captured
                      </div>
                   </div>
                )}
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
                      <ExternalLink size={10} /> Grounding Intelligence:
                    </p>
                      ...
                      {msg.sources.map((source: any, sIdx: number) => (
                        source.web?.uri && (
                          <a 
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-[10px] font-bold text-indigo-400 hover:bg-indigo-500/20 transition-all truncate max-w-[180px] backdrop-blur-md"
                          >
      ...
      <div className="px-4 pb-4">
        <GlassCard className="p-1.5 rounded-full border-white/10 flex items-center gap-2">
          ...
          <button 
            onClick={() => handleSend()} 
            disabled={(!input.trim() && !capturedImage) || isTyping || apiKeyMissing} 
