    ...
    <div className="flex flex-col h-[calc(100vh-180px)] max-w-5xl mx-auto font-sans animate-in fade-in slide-in-from-bottom-6 duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 px-6 gap-6">
        <div className="flex items-center gap-6 text-left">
          <div className="p-4 bg-indigo-500/10 rounded-2xl border border-indigo-500/20 text-indigo-400 shadow-xl">
            <BrainCircuit size={28} />
          </div>
          <div className="space-y-1">
            <h1 className="text-2xl font-black italic text-white uppercase tracking-tight">Neuro Assistant</h1>
            <div className="flex items-center gap-4">
               <NeuralPulse />
               <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest italic">Core Engine Active</span>
            </div>
          </div>
        </div>
      </header>

      <GlassCard className="flex-1 flex flex-col mb-6 overflow-hidden rounded-[3rem] border-white/5 bg-black/40 shadow-2xl" intensity={0.3}>
          ...
          {messages.map((msg, idx) => (
            ...
            >
              ...
              <div className={`space-y-2 max-w-[80%] ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                <div className={`p-8 rounded-[2.5rem] text-base leading-relaxed shadow-2xl transition-all duration-500 ${
                  msg.role === 'assistant' 
                    ? 'bg-slate-900/80 border border-white/5 text-slate-200' 
                    : 'bg-indigo-600 text-white border border-indigo-500'
                }`}>
                  <div className="whitespace-pre-wrap italic font-medium">{String(msg.content || "正在同步神经信号...")}</div>
                  
                  {msg.sources && msg.sources.length > 0 && (
                    <div className="mt-6 pt-6 border-t border-white/10 flex flex-wrap gap-2">
                      {msg.sources.map((src, sIdx) => (
                        <a key={sIdx} href={String(src.web?.uri || '')} target="_blank" className="px-4 py-2 bg-black/40 hover:bg-white/5 border border-white/10 rounded-full text-[10px] font-bold text-indigo-400 flex items-center gap-2 transition-all italic">
        ...
        <div className="p-8 bg-black/60 border-t border-white/5">
          <div className="relative group max-w-4xl mx-auto">
            ...
            <input 
              ...
              className="w-full bg-slate-950 border border-white/10 rounded-full pl-16 pr-20 py-6 text-sm text-white focus:border-indigo-500/50 focus:bg-black outline-none transition-all italic font-bold placeholder:text-slate-800 shadow-inner"
            />
            <m.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => handleSend()}
              disabled={!input.trim() || isTyping}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-4 bg-indigo-600 text-white rounded-full hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-30 shadow-xl"
            >
              <Send size={20} />
            </m.button>
          </div>
