      ...
      <AnimatePresence>
        {showBanner && (
          <m.div 
            ...
            className="relative bg-indigo-600/10 border-b border-indigo-500/20 py-4 px-6 text-center backdrop-blur-md overflow-hidden"
          >
            <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 pr-8">
              <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-indigo-400 italic flex items-center gap-3">
                <BrainCircuit size={16} className="animate-pulse shrink-0" />
                <span>{isZh ? '🧠 加入 SomnoAI 早期访问计划 — 限量测试名额' : '🧠 Join SomnoAI Early Access — Limited Beta Access'}</span>
              </p>
              <m.a 
                ...
                className="px-8 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-500 transition-all text-xs font-black uppercase tracking-widest shadow-lg hover:shadow-indigo-500/25 shrink-0 flex items-center gap-2"
              >
                {isZh ? '立即申请' : 'Apply Now'} <ArrowRight size={14} />
              </m.a>
            </div>
            <button 
              onClick={() => setShowBanner(false)}
              className="absolute right-2 top-2 md:top-1/2 md:-translate-y-1/2 p-2 text-indigo-400/60 hover:text-white hover:bg-white/10 rounded-full transition-all"
            >
              <X size={18} />
            </button>
          </m.div>
      ...
      <footer className="relative z-10 px-12 py-24 flex flex-col md:flex-row justify-between items-center gap-12 bg-[#01040a] border-t border-white/5">
        ...
        <div className="flex flex-col items-center md:items-end gap-8">
          <div className="flex flex-wrap items-center justify-center md:justify-end gap-12">
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('about')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">ABOUT</m.button>
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('opensource')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">ARCHITECTURE</m.button>
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('contact')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">CONTACT</m.button>
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('changelog')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">CHANGELOG</m.button>
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('privacy')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">PRIVACY</m.button>
             <m.button whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} onClick={() => onNavigate('terms')} className="text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors">TERMS</m.button>
          </div>
