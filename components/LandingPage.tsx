...
import { 
  ArrowRight, LogIn, Command, ShieldCheck, Newspaper, FlaskConical, HelpCircle, Info, ArrowUpRight, Play, Volume2, VolumeX, Monitor, Activity, BrainCircuit, Zap, Microscope, LayoutGrid
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#01040a] text-slate-200 font-sans selection:bg-indigo-500/30 overflow-x-hidden relative flex flex-col">
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:40px_40px] opacity-[0.1]" />
        ...
        <div className="max-w-7xl space-y-16 relative z-10">
          ...
          <div className="space-y-4">
            <m.h1 
              ...
              className="text-7xl sm:text-9xl md:text-[11rem] lg:text-[14rem] font-black text-white italic tracking-tighter leading-[0.8] uppercase drop-shadow-2xl"
            >
              Analyze.
            </m.h1>
            <m.h1 
              initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.2 }}
              className="text-7xl sm:text-9xl md:text-[11rem] lg:text-[14rem] font-black text-indigo-600 italic tracking-tighter leading-[0.8] uppercase drop-shadow-[0_0_100px_rgba(79,70,229,0.25)]"
            >
              Recover.
            </m.h1>
          </div>

          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="space-y-8">
        ...
        <div className="max-w-6xl mx-auto space-y-24">
           <div className="text-center space-y-4">
              <div className="inline-flex items-center gap-3 px-5 py-2 bg-indigo-500/5 border border-indigo-500/10 rounded-full">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Laboratory Core Pillars</span>
              </div>
              <h2 className="text-4xl md:text-7xl font-black italic text-white uppercase tracking-tighter">Comprehensive <span className="text-indigo-400">Synthesis</span></h2>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { key: 'telemetry', icon: Activity, color: 'text-rose-500', bg: 'bg-rose-500/5' },
                { key: 'synthesis', icon: BrainCircuit, color: 'text-indigo-400', bg: 'bg-indigo-500/5' },
                { key: 'protocols', icon: Zap, color: 'text-emerald-400', bg: 'bg-emerald-500/5' }
              ].map((p, i) => (
                <GlassCard key={i} className="p-12 rounded-[4rem] border-white/5 bg-slate-900/40 hover:border-indigo-500/30 transition-all group" intensity={1.1}>
                   <div className={`w-16 h-16 rounded-3xl ${p.bg} ${p.color} flex items-center justify-center mb-10 group-hover:scale-110 transition-transform shadow-inner`}>
                      <p.icon size={32} />
                   </div>
                   <h3 className="text-xl font-black italic text-white uppercase tracking-tight mb-4">{t.pillars[p.key].title}</h3>
      ...
      <section className="relative z-10 py-40 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between gap-10 mb-16 px-4">
             <div className="space-y-4 text-left">
                <div className="flex items-center gap-3 text-indigo-400">
                   <Microscope size={20} />
                   <span className="text-[10px] font-black uppercase tracking-[0.4em] italic">Laboratory Visualization</span>
                </div>
                <h2 className="text-4xl md:text-7xl font-black italic text-white uppercase tracking-tighter leading-none">Neural <span className="text-indigo-400">Cinematics</span></h2>
             </div>
             <p className="text-sm md:text-base text-slate-500 italic max-w-sm font-medium leading-relaxed border-l border-white/10 pl-8">
               Watch the reconstruction of sleep architecture in real-time as the Gemini engine processes biological telemetry.
             </p>
          </div>
          ...
          <GlassCard className="rounded-[4rem] md:rounded-[6rem] border-white/10 bg-black/60 overflow-hidden relative group aspect-video shadow-[0_100px_200px_-50px_rgba(0,0,0,1)]" intensity={0.5}>
             ...
             <div className="absolute inset-0 bg-gradient-to-t from-[#01040a] via-transparent to-transparent opacity-80 pointer-events-none" />
             ...
             <div className="absolute bottom-10 left-10 md:bottom-16 md:left-16 right-10 md:right-16 flex items-center justify-between z-20">
                <div className="flex items-center gap-6">
                   ...
                   <div className="space-y-1">
                      <p className="text-[10px] font-black text-white uppercase tracking-widest italic">Signal Encoding</p>
                      <div className="flex items-center gap-2">
                         <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                         <p className="text-[9px] font-mono text-emerald-400 uppercase tracking-widest">ENCODING_RECOVERY_0xFA</p>
                      </div>
                   </div>
                </div>
      ...
      <footer className="relative z-10 px-12 py-24 flex flex-col md:flex-row justify-between items-center gap-12 border-t border-white/5 bg-[#01040a] opacity-60">
        ...
        <div className="flex flex-wrap items-center justify-center gap-10">
           <button onClick={() => onNavigate('opensource')} className="text-[9px] font-black text-slate-500 hover:text-white uppercase tracking-widest italic transition-colors underline decoration-indigo-500/20 underline-offset-8">HYBRID ARCHITECTURE</button>
