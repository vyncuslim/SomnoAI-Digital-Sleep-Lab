...
export const LandingPage: React.FC<LandingPageProps> = ({ lang, onNavigate }) => {
  ...
  const navLinks = [
    { label: isZh ? '科学原理' : 'Scientific Science', view: 'science' },
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="hidden sm:flex items-center gap-2 px-8 py-3 rounded-full bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-white hover:bg-white/10 hover:border-white/20 transition-all active:scale-95"
            >
              <LogIn size={14} className="text-indigo-400" /> {isZh ? '立即登录' : 'Enter Lab'}
            </button>
      ...
      <main>
          ...
          >
            ...
            <div className="space-y-10">
              <h1 className="text-6xl md:text-8xl lg:text-[10rem] font-black italic tracking-tighter text-white uppercase leading-[0.8] drop-shadow-2xl">
                {isZh ? '让每一次睡眠' : 'ENGINEER'} <br/>
                <span className="text-indigo-500">{isZh ? '都成为能量补给' : 'RECOVERY'}</span>
              </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-6 px-4">
              <button 
                ...
                className="w-full sm:w-auto px-16 py-8 bg-indigo-600 text-white rounded-full font-black text-[12px] uppercase tracking-[0.4em] shadow-[0_20px_60px_rgba(79,70,229,0.4)] transition-all hover:scale-105 hover:bg-indigo-500 active:scale-95 italic flex items-center justify-center gap-4"
              >
              ...
              >
                <LogIn size={18} /> {isZh ? '登录控制台' : 'ACCESS TERMINAL'}
              </button>
            </div>
            ...
            <div className="pt-16 flex flex-wrap items-center justify-center gap-12 opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
               ...
               <div className="flex items-center gap-3">
                  <Fingerprint size={20} />
                  <span className="text-[10px] font-black uppercase tracking-[0.4em]">Edge Computing</span>
               </div>
            </div>
          </m.div>
        </section>

        {/* 4. LABORATORY PREVIEW (Rich Semantic Section) */}
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          ...
                          <div className="bg-slate-900/60 rounded-[3rem] p-12 border border-white/5 min-h-[400px] space-y-10">
                             ...
                             <p className="text-[11px] text-slate-500 italic leading-relaxed pt-4 border-t border-white/5">
                                "Our AI reconstructs sleep stages (Deep, REM, Light) using Google Gemini's advanced reasoning models."
                             </p>
                          </div>
                       </div>
                    </div>
        ...
        <section className="py-32 px-4 max-w-7xl mx-auto space-y-20">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <FeatureCard 
                ...
                desc={isZh ? "通过 Health Connect 同步可穿戴设备数据，或通过手动终端注入您的主观感受。我们支持主流传感器协议。" : "Sync wearable data via Health Connect or use our Injection Terminal for manual biological logging. Sensor-agnostic infrastructure."}
                delay={0.1}
              />
              <FeatureCard 
                icon={BrainCircuit}
                title={isZh ? "2. 神经模型合成" : "2. Neural Synthesis"}
                desc={isZh ? "利用 Google Gemini AI 模型识别深睡效率与 REM 节奏，精确解构您的夜间恢复架构。" : "Leverage Google Gemini neural models to identify deep sleep efficiency and REM rhythms, reconstructing your restoration architecture."}
                delay={0.2}
              />
              <FeatureCard 
                icon={Sparkles}
                title={isZh ? "3. 精准优化协议" : "3. Precision Protocol"}
                desc={isZh ? "获取量身定制的睡眠优化方案，锁定并消除破坏因素，将每一次休息转化为高性能能量储备。" : "Receive tailored optimization protocols. Identify disruptors and transform every rest period into a high-performance energy reserve."}
                delay={0.3}
              />
           </div>
        </section>
        ...
        <section className="py-32 bg-indigo-600/5 border-y border-white/5">
           <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              <div className="space-y-12">
                 <h2 className="text-4xl md:text-6xl font-black italic text-white uppercase tracking-tighter leading-none">Why <span className="text-indigo-500">SomnoAI</span>?</h2>
                 ...
                 <div className="space-y-8">
                    {[
                      { icon: Lock, title: "Edge-First Privacy", desc: "No biological data touches our permanent cloud servers. Everything is processed locally." },
                      { icon: Cpu, title: "Gemini-Powered", desc: "Advanced reasoning that understands the 'Why' behind your sleep patterns, not just 'What'." },
                      { icon: Globe, title: "Open Link", desc: "Sync data from any source: Oura, Apple, Garmin, or manual entry. Total data sovereignty." }
                    ].map((item, i) => (
              ...
              <GlassCard className="p-12 rounded-[4rem] border-white/5 bg-black/40 overflow-hidden" intensity={1.5}>
                 <div className="space-y-10 relative z-10">
                    <div className="flex items-center gap-3">
                       ...
                       <h3 className="text-[11px] font-black uppercase text-white tracking-[0.3em] italic">Lab Testimonial</h3>
                    </div>
                    <p className="text-2xl font-black italic text-indigo-300 leading-snug">"The first platform that actually explained *why* my deep sleep was fragmented instead of just giving me a generic score."</p>
                    <div className="flex items-center gap-5">
                       <div className="w-12 h-12 rounded-full bg-indigo-600 flex items-center justify-center text-white font-black italic">S</div>
                       <div>
                          <p className="text-sm font-black text-white uppercase tracking-widest italic">Subject #2481</p>
                          <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Early Access User</p>
                       </div>
                    </div>
                 </div>
        ...
        <section className="py-40 text-center px-4 relative overflow-hidden">
           ...
           >
              ...
              <p className="text-slate-500 font-bold uppercase tracking-[0.5em] text-[10px] italic">No Hardware Required • AI Neural Core Ready</p>
              <div className="flex justify-center">
                 ...
                 >
                   Launch Lab Terminal <ArrowRight size={20} />
                 </button>
              </div>
           </m.div>
        </section>
        ...
        <div className="max-w-7xl mx-auto space-y-24">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16">
            <div className="space-y-8">
               ...
               <p className="text-xs text-slate-500 italic font-medium leading-relaxed max-w-xs">
                 Dedicated to the science of recovery and high-performance restoration. Built with neural precision. Not affiliated with REMspace.
               </p>
               <div className="flex gap-4">
                  {[
            ...
            <div className="space-y-8">
               <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-400">Compliance</h4>
               <ul className="space-y-4 text-xs font-bold text-slate-500 italic">
                 ...
                 <li><a href="https://developers.google.com/terms/api-services-user-data-policy" target="_blank" className="hover:text-white transition-colors uppercase tracking-widest flex items-center gap-2">Google Disclosure <ExternalLink size={10} /></a></li>
               </ul>
            </div>
            ...
            <div className="space-y-8">
               ...
               <div className="p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] space-y-4">
                  <div className="flex items-center justify-between">
                     <span className="text-[9px] font-black text-slate-600 uppercase">Neural Link</span>
                  ...
                  <div className="flex items-center justify-between">
                     <span className="text-[9px] font-black text-slate-600 uppercase">API Node</span>
                     <span className="text-[9px] font-black text-indigo-400 uppercase">Gemini 2.5 Pro</span>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40">
             <div className="flex flex-col items-center md:items-start gap-2">
                <p className="text-[9px] font-mono uppercase tracking-[0.5em] text-slate-400">@2026 SomnoAI Digital Sleep Lab • Secure Ingress</p>
                <p className="text-[8px] font-black uppercase text-slate-600 tracking-[0.2em] italic">Privacy-focused recovery architecture built in Penang, Malaysia.</p>
             </div>
             <div className="flex items-center gap-2 text-indigo-400 px-6 py-2 bg-indigo-500/5 rounded-full border border-indigo-500/10">
                <ShieldCheck size={12} />
                <span className="text-[9px] font-black uppercase tracking-widest italic">Encrypted Secure Ingress Active</span>
             </div>
          </div>
        </div>
      </footer>
