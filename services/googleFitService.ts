...
export class GoogleFitService {
  ...
  async fetchSleepData(): Promise<Partial<SleepRecord>> {
    ...
    try {
      ...
      if (dsRes.status === 403) throw new Error("PERMISSION_DENIED: 权限未授予");
      ...
      if (sleepPoints.length === 0) {
        // 这可能是权限问题（能登录但没给读权限）或真的没数据
        throw new Error("DATA_NOT_FOUND: 未检测到睡眠信号，请检查权限勾选或手机同步状态");
      }
      ...
      let remMins = 0;
      let awakeMins = 0;
      const stages: SleepStage[] = sleepPoints.filter(p => 
        ...
        return { name: type === 5 ? '深睡' : type === 6 ? 'REM' : type === 1 ? '清醒' : '浅睡', duration, startTime: '' } as SleepStage;
      });

      return {
        totalDuration,
        score: Math.min(100, Math.round((totalDuration / 480) * 100)),
        ...
        stages,
        heartRate: { resting: 60, average: 65, min: 55, max: 85, history: [] },
        aiInsights: ["实验室：生理信号流同步成功。"]
      };
